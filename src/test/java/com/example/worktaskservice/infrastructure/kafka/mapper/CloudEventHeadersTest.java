...
class CloudEventHeadersTest {
    ...
    void tagsEventsWithUrnSubjectAndCorrelation() {
        ...
        Headers headers = mapper.toAvro(event, task, null, null, causationId, "urn:source:billing:invoicing")
                .headers();
    ...
    void propagatesInboundSourceAndFallsBackToServiceIdentity() {
        ...
        Headers propagated = mapper.toAvro(event, task(id), null, null, null, "urn:source:billing:invoicing")
                .headers();
        assertEquals("urn:source:billing:invoicing", header(propagated, "ce_source"));

        Headers fallback = mapper.toAvro(event, task(id), null, null, null, null).headers();
    ...
    void tagsRejectionEventsWithCausationToo() {
        ...
        Headers headers = mapper.toAvro(event, task(id), null, null, causationId, null).headers();
    ...
    void omitsCausationWhenInboundIdAbsent() {
        ...
        Headers headers = mapper.toAvro(event, task(id), null, null, null, null).headers();
